...
После этого откройте `http://127.0.0.1:8001/`. В этом режиме интерфейс берет клиентов, справы, задачи, события и финансы из `/api/bootstrap/`. Если открыть обычный статический сервер на `8000`, интерфейс продолжит работать от JSON-демо-данных.
...
Текущий `frontend` нужен, чтобы быстро согласовать интерфейс и логику. Django backend уже запущен как первый рабочий слой:

- SQLite на старте;
- модели клиентов, дел, документов, задач, событий, финансов и рассылок;
- Django Admin для ручной проверки данных;
- API для клиентов, справ, задач, календаря и финансовой сводки;
- команда импорта текущих демо-данных из `frontend/data`.

Дальше нужно соединить frontend с API и добавить:
