...
const calendarToday = "2024-05-15";
...
function calendarViewDays(date, mode) {
  if (mode === "day") {
...
function advocatePhoto(name = "Адвокат", extraClass = "") {
...
function documentActionButtons(source, firstIndex, secondIndex = "") {
...
function renderClients() {
  const selected = clientById(state.selectedClientId) || state.clients[0];
  const telegramConnected = state.clients.filter((client) => client.telegram).length;
  $("#clients").innerHTML = `
  ...
  `;
  renderClientRows();
  renderClientProfile(selected.id);
  bindClientMailingPreview();
...
function renderClientRows() {
  ...
  document.querySelectorAll("[data-client]").forEach((link) => {
    link.addEventListener("click", (event) => {
      ...
      renderClientProfile(link.dataset.client);
    });
  });
...
function bindClientMailingPreview() {
