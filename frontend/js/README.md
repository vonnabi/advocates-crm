...
- `state.js` loads `/api/bootstrap/` when the app is opened through Django or `crmApiBase` is configured, otherwise it falls back to static JSON demo data. Tests can force static demo data with `crmApiMode=static`.
